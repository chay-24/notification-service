...
func Get(db *sqlx.DB) ([]Notification, error) {
	...
	if err := db.Select(&notifications, "SELECT * FROM notifications ORDER BY timestamp DESC LIMIT 100"); err != nil {
...
func Save(db *sqlx.DB, source, title, message string) error {
	_, err := db.Exec(
		"INSERT INTO notifications (source, title, message, timestamp) VALUES (?, ?, ?, datetime('now'))",
